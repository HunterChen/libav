                                  ...
                                  const uint8_t *buf, int buf_size)
{
    ...
    for (;;) {
        ...
        if (buf >= buf_end)
            break;
        h->nal_unit_type = (*(buf - 1) >> 1) & 0x3f;
        h->temporal_id   = (*buf & 0x07) - 1;
        ...
        ptr = ff_hevc_extract_rbsp(h, buf, &dst_length, &consumed, src_length);
        if (ptr == NULL || dst_length < 0)
            break;

        init_get_bits(gb, ptr, 8 * dst_length);
...
static void close(AVCodecParserContext *s)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(h->sps_list); i++)
        av_freep(&h->sps_list[i]);
    for (i = 0; i < FF_ARRAY_ELEMS(h->pps_list); i++)
        ff_hevc_pps_free(&h->pps_list[i]);
}
