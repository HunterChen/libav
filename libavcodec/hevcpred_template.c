...
#define EXTEND_DOWN_CIP(ptr, start, length) \
    ...
    int pic_width_in_min_pu = PU(s->sps->full_width);
    ...
    if (s->pps->constrained_intra_pred_flag == 1) {
        if (cand_bottom_left || cand_left || cand_up_left || cand_up || cand_up_right) {
            int size_max_x = x0 + ((2 * size) << hshift) < s->sps->full_width ?
                                    2 * size : (s->sps->full_width - x0) >> hshift;
            int size_max_y = y0 + ((2 * size) << vshift) < s->sps->full_height ?
                                    2 * size : (s->sps->full_height - y0) >> vshift;
